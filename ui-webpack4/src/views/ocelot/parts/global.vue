    ...
    <Form :model="vmodel" :label-width="150">
      ...
      <Card class="mar10" dis-hover>
        ...
        <FormItem label="ConfigurationKey">
          <Input
            v-model="vmodel.ServiceDiscoveryProvider.ConfigurationKey"
          ></Input>
        </FormItem>
        <FormItem label="PollingInterval">
          <Input
            v-model="vmodel.ServiceDiscoveryProvider.PollingInterval"
            number
          ></Input>
        </FormItem>
      </Card>
      <Card class="mar10" dis-hover>
        ...
        <FormItem label="RateLimitCounterPrefix">
          <Input
            v-model="vmodel.RateLimitOptions.RateLimitCounterPrefix"
          ></Input>
        </FormItem>
        <FormItem label="DisableRateLimitHeaders">
          <i-switch
            v-model="vmodel.RateLimitOptions.DisableRateLimitHeaders"
          ></i-switch>
        </FormItem>
        <FormItem label="HttpStatusCode">
          <Input
            v-model="vmodel.RateLimitOptions.HttpStatusCode"
            number
          ></Input>
        </FormItem>
      </Card>
...
export default {
  ...
  props: {
    vmodel: {
      type: Object,
      default() {
        return modelTempl.getOcelotConfigurationSchema().GlobalConfiguration
      }
    }
  },
  mounted() {},
