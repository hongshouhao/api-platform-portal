  ...
  <div>
    <Form :model="vmodel"
      ...
      <Row>
        ...
        <Col span="12">
        <FormItem label="DangerousAcceptAnyServerCertificateValidator"
                  :label-width="300">
          <i-switch v-model="vmodel.DangerousAcceptAnyServerCertificateValidator"></i-switch>
        </FormItem>
        </Col>
      </Row>
      <FormItem label="UpstreamHttpMethod"
                class="highlight">
        <Row>
          <Col span="1">
          <Tooltip placement="right"
          ...
          <Col span="23">
          <HttpMethodEditor :httpMethods.sync="vmodel.UpstreamHttpMethod"></HttpMethodEditor>
          </Col>
        </Row>
      </FormItem>

      <port-view class="mar10 highlight"
                 title="DownstreamHostAndPorts"
                 tooltip="原始服务IP和端口"
                 :array="vmodel.DownstreamHostAndPorts"></port-view>

      <Card class="mar10 highlight"
            dis-hover>
        <p slot="title">
        ...
        <FormItem label="AuthenticationProviderKey"
          ...
          <Row>
            ...
            <Col span="23">
            <Select v-model="vmodel.AuthenticationOptions.AuthenticationProviderKey"
                    :loading="loadingAuthOptions">
              <Option v-for="(item, index) in options"
                      :value="item.AuthScheme"
                      :key="index">{{item.AuthScheme}}</Option>
            </Select>
            </Col>
          </Row>
        </FormItem>
        <FormItem label="AllowedScopes"
                  class="highlight">
          <Row>
            <Col span="1">
            <Tooltip placement="right"
            ...
            <Col span="23">
            <ScopeEditor :idsHost="url"
                         :scopes.sync="vmodel.AuthenticationOptions.AllowedScopes"></ScopeEditor>
            </Col>
          </Row>
        </FormItem>
      </Card>
      <ClaimEditor class="mar10"
                   title="RouteClaimsRequirement"
                   :property.sync="vmodel.RouteClaimsRequirement"
                   :idsHost="url"></ClaimEditor>
      <KeyValueEditor class="mar10"
                      title="UpstreamHeaderTransform"
                      :property.sync="vmodel.UpstreamHeaderTransform"></KeyValueEditor>
      <KeyValueEditor class="mar10"
                      title="DownstreamHeaderTransform"
                      :property.sync="vmodel.DownstreamHeaderTransform"></KeyValueEditor>
      <KeyValueEditor class="mar10"
                      title="AddHeadersToRequest"
                      :property.sync="vmodel.AddHeadersToRequest"></KeyValueEditor>
      <KeyValueEditor class="mar10"
                      title="AddClaimsToRequest"
                      :property.sync="vmodel.AddClaimsToRequest"></KeyValueEditor>
      <KeyValueEditor class="mar10"
                      title="AddQueriesToRequest"
                      :property.sync="vmodel.AddQueriesToRequest"></KeyValueEditor>
      <Card class="mar10"
            dis-hover>
        <p slot="title">FileCacheOptions</p>
        <FormItem label="TtlSeconds">
          <Input v-model="vmodel.FileCacheOptions.TtlSeconds"
                 number></Input>
        </FormItem>
      ...
      <Card class="mar10"
        ...
        <FormItem label="expiry">
          <Input v-model="vmodel.LoadBalancerOptions.Expiry"
                 number></Input>
        </FormItem>
      </Card>
      <Card class="mar10"
        ...
        <FormItem label="ExceptionsAllowedBeforeBreaking"
                  :label-width="200">
          <Input v-model="vmodel.QoSOptions.ExceptionsAllowedBeforeBreaking"
                 number></Input>
        </FormItem>
        <FormItem label="DurationOfBreak"
                  :label-width="200">
          <Input v-model="vmodel.QoSOptions.DurationOfBreak"
                 number></Input>
        </FormItem>
        <FormItem label="TimeoutValue"
                  :label-width="200">
          <Input v-model="vmodel.QoSOptions.TimeoutValue"
                 number></Input>
        </FormItem>
      </Card>
      ...
      <Card class="mar10"
        ...
        <Row>
          ...
          <FormItem label="Limit">
            <Input v-model="vmodel.RateLimitOptions.Limit"
                   number></Input>
          </FormItem>
          </Col>
        </Row>
        <Row>
          ...
          <FormItem label="PeriodTimespan">
            <Input v-model="vmodel.RateLimitOptions.PeriodTimespan"
                   number></Input>
          </FormItem>
          </Col>
        </Row>
        <StringList title="ClientWhitelist"
                    :array="vmodel.RateLimitOptions.ClientWhitelist"></StringList>
      </Card>
      ...
      <StringList class="mar10"
                  ...
                  :array="vmodel.DelegatingHandlers"></StringList>
      ...
      <Card class="mar10"
        ...
        <StringList class="mar10"
                    title="IpAllowedList"
                    :array="vmodel.SecurityOptions.IpAllowedList"></StringList>
        <StringList class="mar10"
                    title="IpBlockedList"
                    :array="vmodel.SecurityOptions.IpBlockedList"></StringList>
      </Card>
    </Form>
  </div>
</template>
    ...
    url: {
      get () {
        ...
        var slted = Enumerable.from(_this.options).firstOrDefault(
          s => s.AuthScheme === _this.vmodel.AuthenticationOptions.AuthenticationProviderKey
        )
        if (slted) {
