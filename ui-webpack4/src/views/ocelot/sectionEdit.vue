<template>
  <div>
    ...
    <Tabs :value="defaultTab" @on-click="tabChanged" style="margin:5px 0;">
      <Select
        placeholder="模板"
        slot="extra"
        v-model="sltedTempl"
      ...
      <Button
        ...
        >{{ configType }}</Button
      >
      <TabPane label="Global" v-if="vsection.isGlobal" name="Global">
        <GlobalConfigView
          class="content"
          :vmodel="vconfig.GlobalConfiguration"
        ></GlobalConfigView>
      </TabPane>
      <TabPane label="ReRoutes" v-if="!vsection.isGlobal" name="ReRoute">
        <Collapse v-model="slctedReRoutePanel" accordion>
          <template v-for="(item, index) in vconfig.ReRoutes">
            <Panel
              :key="'reRoute' + index.toString()"
  ...
  methods: {
    ...
    sltedTemplChanged() {
      if (this.vsection.isGlobal) {
        this.vconfig.GlobalConfiguration = JSON.parse(
          this.sltedTempl
        ).GlobalConfiguration
      }
    },
